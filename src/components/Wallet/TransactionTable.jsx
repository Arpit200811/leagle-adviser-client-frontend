...
const TransactionTable = ({ transactions = [] }) => {
    // If no real transactions, show a "No transactions found" state or map existing ones
    const displayTransactions = transactions.map(t => ({
        id: t.id,
        title: t.description || "Transaction",
        info: t.referenceId ? `Ref: ${t.referenceId}` : "Internal Transfer",
        date: new Date(t.createdAt).toLocaleDateString(),
        time: t.createdAt ? new Date(t.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '',
        type: t.type === 'credit' ? 'Deposit' : 'Service Fee',
        status: t.status === 'success' ? 'Completed' : 'Pending',
        amount: t.type === 'credit' ? Number(t.amount) : -Number(t.amount),
        icon: t.type === 'credit' ? <HiOutlinePlusCircle /> : <HiOutlineDownload />,
        color: t.type === 'credit' ? "green" : "blue"
    }));
